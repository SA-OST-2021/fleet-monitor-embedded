...
#define __GP_REGISTER_N(name, address, size)                                                \
  static uint16_t write##name(const uint8_t *_buff) { return write(address, _buff, size); } \
  static uint16_t read##name(uint8_t *_buff) { return read(address, _buff, size); }
...
#undef __GP_REGISTER_N
 ...
 private:
  ...
  static inline uint8_t readSn(SOCKET s, uint16_t addr) { return read(CH_BASE() + s * CH_SIZE + addr); }
...
#define __SOCKET_REGISTER8(name, address)                                                   \
  static inline void write##name(SOCKET _s, uint8_t _data) { writeSn(_s, address, _data); } \
...
#define __SOCKET_REGISTER_N(name, address, size)                                                       \
  static uint16_t write##name(SOCKET _s, uint8_t *_buff) { return writeSn(_s, address, _buff, size); } \
  static uint16_t read##name(SOCKET _s, uint8_t *_buff) { return readSn(_s, address, _buff, size); }
...
#undef __SOCKET_REGISTER_N
 ...
 public:
...
#else
  static const uint16_t SSIZE = 4096;
  ...
  static void setSpiPins(uint8_t clk, uint8_t miso, uint8_t mosi) {
    clk_pin = clk;
    miso_pin = miso;
    mosi_pin = mosi;
  }
 ...
 private:
  ...
  inline static void setSS() { *(ss_pin_reg + 34) = ss_pin_mask; }
  inline static void resetSS() { *(ss_pin_reg + 33) = ss_pin_mask; }
