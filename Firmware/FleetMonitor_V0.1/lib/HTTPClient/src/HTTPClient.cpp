...
class TransportTraits {
 public:
  ...
  virtual std::unique_ptr<WiFiClient> create() { return std::unique_ptr<WiFiClient>(new WiFiClient()); }
...
class TLSTraits : public TransportTraits {
 public:
  ...
  std::unique_ptr<WiFiClient> create() override { return std::unique_ptr<WiFiClient>(new WiFiClientSecure()); }
...
bool HTTPClient::begin(String host, uint16_t port, String uri, const char* CAcert, const char* cli_cert,
                       const char* cli_key) {
  if (_client && !_tcpDeprecated) {
    log_d("mix up of new and deprecated api");
    _canReuse = false;
 ...
 * @param reuse bool
 */
void HTTPClient::setReuse(bool reuse) { _reuse = reuse; }
 ...
 */
void HTTPClient::setConnectTimeout(int32_t connectTimeout) { _connectTimeout = connectTimeout; }
 ...
 * @return http code
 */
int HTTPClient::POST(uint8_t* payload, size_t size) { return sendRequest("POST", payload, size); }

int HTTPClient::POST(String payload) { return POST((uint8_t*)payload.c_str(), payload.length()); }
 ...
 * @return http code
 */
int HTTPClient::PATCH(uint8_t* payload, size_t size) { return sendRequest("PATCH", payload, size); }

int HTTPClient::PATCH(String payload) { return PATCH((uint8_t*)payload.c_str(), payload.length()); }
 ...
 * @return http code
 */
int HTTPClient::PUT(uint8_t* payload, size_t size) { return sendRequest("PUT", payload, size); }

int HTTPClient::PUT(String payload) { return PUT((uint8_t*)payload.c_str(), payload.length()); }
 ...
 * @return -1 if no info or > 0 when Content-Length is set by server
 */
int HTTPClient::getSize(void) { return _size; }
...
int HTTPClient::headers() { return _headerKeysCount; }
...
void HTTPClient::setFollowRedirects(followRedirects_t follow) { _followRedirects = follow; }

void HTTPClient::setRedirectLimit(uint16_t limit) { _redirectLimit = limit; }
...
const String& HTTPClient::getLocation(void) { return _location; }
